...
export const Blog: React.FC<BlogProps> = ({ initialPostId, onBack }) => {
    ...
    // 1. Fetch list on load & Fetch specific post if ID provided
    useEffect(() => {
        fetchPosts();
    ...
    // 👇 THIS IS THE NEW FUNCTION TO HANDLE "BACK" LOGIC
    const handleBack = () => {
        setSelectedPost(null); // Clear local selection
        if (onBack) onBack();  // Clear ID in parent (App.tsx)
    };
